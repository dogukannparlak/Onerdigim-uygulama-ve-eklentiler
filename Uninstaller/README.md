# 🗑️ Kaldırma Araçları Koleksiyonu

> **Kısa Açıklama:** Windows'un yerleşik kaldırıcısının bıraktığı dosya ve kayıt defteri artıklarını temizlemek için Revo Uninstaller ve Bulk Crap Uninstaller rehberleri.

Windows · Ücretsiz seçenekler mevcut · Sistem araçları

---

## 📌 Genel Bakış

Windows "Uygulamalar ve Özellikler" yalnızca program kaydını siler; kalıntılar çoğu zaman diskte ve kayıt defterinde kalır. Bu klasörde iki popüler üçüncü taraf kaldırıcı ve aralarındaki karşılaştırma rehberi bulunur.

**Hızlı öneri:**

- Tek tek program, Hunter Mode, adım adım artık onayı → **Revo Uninstaller**
- Onlarca program, bloatware temizliği, ücretsiz toplu kaldırma → **Bulk Crap Uninstaller (BCU)**

Karar veremiyorsan önce [karşılaştırma rehberine](Revo-vs-Bulk-Crap-Uninstaller.md) bak.

---

## 📂 Rehberler

| Rehber | Ne için? |
| ------ | -------- |
| [Revo-Uninstaller .md](Revo-Uninstaller%20.md) | Derin artık taraması, Hunter Mode, Free vs Pro |
| [Bulk-Crap-Uninstaller .md](Bulk-Crap-Uninstaller%20.md) | Toplu kaldırma, Steam/UWP/portable algılama, ücretsiz |
| [Revo-vs-Bulk-Crap-Uninstaller.md](Revo-vs-Bulk-Crap-Uninstaller.md) | Hangisini ne zaman kullanmalısın? |

---

## ✨ Kısa Özet

### Revo Uninstaller

- Kaldırma sonrası **Güvenli / Orta / Gelişmiş** artık taraması
- **Hunter Mode** — simge veya pencereye sürükleyerek program bul ve kaldır
- **Free** çoğu kullanıcıya yeter; **Pro** zorla kaldırma, toplu silme ve kurulum izleme ekler

### Bulk Crap Uninstaller (BCU)

- **Toplu kaldırma** — birden fazla programı tek seferde sil
- Steam, UWP, portable, Chocolatey ve gizli uygulama algılama
- Tamamen **ücretsiz** ve açık kaynak

---

## ⚠️ Kullanmadan önce

1. **Geri yükleme noktası** oluştur
2. Windows sistem bileşenlerine ve bilmediğin kayıt defteri girdilerine dokunma
3. İki aracı aynı anda çalıştırma; kaldırma işlemlerini sırayla yap
4. Artık taramada yalnızca programla ilgili olduğundan emin olduğun girdileri sil

---

## 🔗 Resmi Siteler

- [Revo Uninstaller](https://www.revouninstaller.com/)
- [BCU — bcuninstaller.com](https://www.bcuninstaller.com/)
- [BCU — GitHub](https://github.com/Klocman/Bulk-Crap-Uninstaller)
- [BCU — SourceForge](https://sourceforge.net/projects/bulk-crap-uninstaller/)

---

## 📝 Notlar

> Bu klasörde 3 rehber vardır: Revo, BCU ve ikisi arası karşılaştırma. Detaylı kurulum, kullanım adımları ve bilinen sorunlar ilgili rehber dosyalarında anlatılır.

---

## ⚠️ Sorumluluk Reddi

Bu repository yalnızca bilgilendirme amaçlıdır. Burada önerilen uygulamalar ve eklentiler:

- **Kendi sorumluluğunuzda kullanın**: Uygulamaların sisteminizde neden olabileceği herhangi bir sorun, veri kaybı veya sistem hasarından sorumlu değiliz
- **Resmi kaynaklardan indirin**: Mutlaka uygulamaları resmi web sitelerinden veya güvenilir kaynaklardan indirin
- **Güncellik garantisi yoktur**: Uygulama bilgileri zaman içinde güncelliğini yitirebilir
- **Virüs/malware kontrolü yapın**: İndirdiğiniz dosyaları güvenlik yazılımınızla tarayın
- **Sistem yedeklemesi alın**: Önemli verilerinizi yedeklemeden yeni yazılım kurmayın
- **Lisans koşullarına dikkat edin**: Her uygulamanın kendi lisans koşulları vardır
- **Kişisel veri güvenliği**: Uygulamaların gizlilik politikalarını inceleyin

**Kullanım öncesi mutlaka araştırma yapın ve bu uygulamaları kendi riskinizle kullanın.**

---

*Son güncelleme: 2026-05-23*
